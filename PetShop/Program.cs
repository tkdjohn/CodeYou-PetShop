...
static T? GetEntityFromUser<T>() where T : BaseEntity {
...
static bool ValidateEntity<T>(T entity) where T : BaseEntity {
...
async Task AddEntity<T>(T? newEntity) where T : BaseEntity {
    if (newEntity == null) {
...
async Task AddUpdateEntity<T>(T? entityUpdate) where T : BaseEntity {
